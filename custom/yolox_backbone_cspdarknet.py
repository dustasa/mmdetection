...
def yolox_cspdarknet_s(pretrained=False):
    # build backbone
    backbone = CSPDarknet(deepen_factor=0.33, widen_factor=0.5, depthwise=False, act='silu')
...
def yolox_cspdarknet_m(pretrained=False):
    # build backbone
    backbone = CSPDarknet(deepen_factor=0.67, widen_factor=0.75, depthwise=False, act='silu')
...
def yolox_cspdarknet_l(pretrained=False):
    # build backbone
    backbone = CSPDarknet(deepen_factor=1.0, widen_factor=1.0, depthwise=False, act='silu')
...
def yolox_cspdarknet_x(pretrained=False):
    # build backbone
    backbone = CSPDarknet(deepen_factor=1.33, widen_factor=1.25, depthwise=False, act='silu')
...
def yolox_cspdarknet_tiny(pretrained=False):
    # build backbone
    backbone = CSPDarknet(deepen_factor=0.33, widen_factor=0.375, depthwise=False, act='silu')
...
def yolox_cspdarknet_nano(pretrained=False):
    # build backbone
    backbone = CSPDarknet(deepen_factor=0.33, widen_factor=0.25, depthwise=True, act='silu')

    # load weight
    if pretrained:
        print('Loading pretrained cspdarknet_nano ...')
        path_to_dir = os.path.dirname(os.path.abspath(__file__))
        path_to_weight = path_to_dir + '/weights/yolox_cspdarknet_nano.pth'
        checkpoint = torch.load(path_to_weight, map_location='cpu')
        backbone.load_state_dict(checkpoint)
