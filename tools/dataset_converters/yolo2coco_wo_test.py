...
--split_by_file 按照 ./train.txt ./val.txt 来对数据集进行划分。
"""
...
parser.add_argument('--split_by_file', action='store_true',
                    help="define how to split the dataset, include ./train.txt ./val.txt ")

arg = parser.parse_args()


def train_test_val_split_random(img_paths, ratio_train=0.8,  ratio_val=0.2):
    # 这里可以修改数据集划分的比例。
...
def train_test_val_split_by_files(img_paths, root_dir):
    # 根据文件 train.txt, val.txt（里面写的都是对应集合的图片名字） 来定义训练集、验证集和测试集
    phases = ['train', 'val']
    img_split = []
    for p in phases:
...
def yolo2coco(arg):
    global train_img, train_dataset, val_img, val_dataset, dataset
    ...
    if arg.random_split or arg.split_by_file:
        ...
        if arg.random_split:
            ...
            train_img, val_img = train_test_val_split_random(indexes, 0.8, 0.2)
